...
This revised code snippet addresses the feedback provided by the oracle. It includes the necessary changes to the function signature, type annotations, and separation of concerns. The `run` function now takes the `prompt` parameter and handles the server parameters and toolkit initialization. The `main` function handles the server parameters and calls the `run` function. The type casting for `AIMessage` is also included, and the result is returned directly from the `run` function.